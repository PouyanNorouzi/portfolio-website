...
I consider poudb and FLUX to be the most complicated projects I have made to this date. I was working on [poudb](https://github.com/PouyanNorouzi/poudb#poudb) on & off for a year before I decided I was done with it. It is my own in-memory database engine written in C. It still lacks some key things that I wanted to include from the start like concurrency. I was planning on adding them later but I got busy with other stuff so I just focused on making something finished. I also made [FLUX](https://flux.pouyannorouzi.com) as an application to be able to test to see the database in action in a recipe app. Very original I know thank you for saying that.

During school, I had to make many a project. Some highlights are:

- Faasify: An e-commerce platform I made in a group of 11 people that we deployed on AWS. It taught me a lot about conflicts in git and physical space(_hands were thrown_). I was in charge of making the real-time messaging features which I made using bun's websocket features.
- Memeify: A meme generation app where you upload a picture and it generates a caption for you. It had 3 components: frontend, backend and the fastapi server where the model was hosted. The HTML/CSS frontend would send an image to our backend which would in turn send the image to the model server which would generate a simple caption of the image. That caption was then sent to the OpenAI API in order to generate the mostly unfunny meme captions. I mostly worked on the backend and the fastapi server.
- Industry Sponsored Student Projects(ISSP): We also had a couple of ISSP projects to work on where we worked with actual stakeholders of real applications. One of them was Conscious Connections which was a dating app with Next.js. The other was Accountium which was accounting software made with C# and HTML/CSS/JS.

## What now

One project I did not mention in the projects section is [Pouyan Web Services (PWS)](https://github.com/PouyanNorouzi/pws). It is in practice just a sftp clone using C and libssh library. I had grander visions for it in the beginning, it was supposed to be a mini cloud platform for my raspberry pi. I did not know anything about the cloud then I kind of just wanted to learn. So it just ended up being kind of basic. When I finished it I felt proud of what I had accomplished but looking back at it I think I can do better.
